      ...
      {Array.from({ length: count }).map((_, i) => (
        ...
        >
          <div className="mb-3 flex gap-2">
            <div className="h-6 w-12 rounded bg-[var(--color-surface-2)]" />
            <div className="h-6 w-16 rounded bg-[var(--color-surface-2)]" />
          </div>
          <div className="mb-2 h-5 w-3/4 rounded bg-[var(--color-surface-2)]" />
          <div className="mb-3 h-4 w-full rounded bg-[var(--color-surface-2)]" />
...
export function PageLoader({ message }: { message?: string }) {
  return (
    <div className="flex min-h-[60vh] flex-col items-center justify-center gap-4">
      <Spinner size="lg" />
      <p className="text-sm text-[var(--color-text-2)]">{message ?? "Loading..."}</p>
    </div>
  );
}
